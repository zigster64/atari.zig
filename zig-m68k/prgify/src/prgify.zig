...
pub fn convert(alloc: Allocator, io: std.Io, elf_path: []const u8, prg_path: []const u8) !void {
    ...
    _ = start_vma; // unused — we always use 0x601A magic

    var header: [28]u8 = [_]u8{0} ** 28;
    header[0] = 0x60;
    header[1] = 0x1a; // 0x601A = GEMDOS magic, branches to text+0
    ...
    std.mem.writeInt(i16, header[26..28], 1, .big); // absflag: position-independent
