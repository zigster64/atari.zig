...
noinline fn aesCall(opcode: AesOpcode, args: *const AesArgs) i16 {
    ...
    // Stack-local AES global block. A module-level block would sit after the
    // framebuffers in .bss (>32 KB) and its PC-relative reference would be
    // misresolved by toslink (M68K_NOTES #11). The apps read their id from
    // the appl_init RETURN value, not this block; appl_read/appl_write (which
    // would need the persisted id) are not part of the current surface.
...
/// TEMP DEBUG: print a 32-bit value as 8 uppercase hex digits to the GEMDOS
/// console. Companion to `dbg` for values that are not comptime (runtime
/// scancodes etc.).
///
/// Fully unrolled with fixed-offset stores: a loop with a computed index
/// (`buf[7 - i] = ...`) miscompiled on the m68k backend — every digit was
/// stored to the same byte (verified: the output printed a single `0`).
pub fn dbgHex(v: u32) void {
    const hex: []const u8 = "0123456789ABCDEF";
    var buf: [11]u8 = undefined; // 8 digits + CR + LF + NUL
    buf[0] = hex[@intCast((v >> 28) & 0xF)];
    buf[1] = hex[@intCast((v >> 24) & 0xF)];
    buf[2] = hex[@intCast((v >> 20) & 0xF)];
    buf[3] = hex[@intCast((v >> 16) & 0xF)];
    buf[4] = hex[@intCast((v >> 12) & 0xF)];
    buf[5] = hex[@intCast((v >> 8) & 0xF)];
    buf[6] = hex[@intCast((v >> 4) & 0xF)];
    buf[7] = hex[@intCast(v & 0xF)];
    buf[8] = '\r';
    buf[9] = '\n';
    buf[10] = 0;
...
/// 32-bit store through a plain `(a0)` address (same rationale as `storeWord`;
/// used by the fast long-based framebuffer fills).
pub inline fn storeLong(dest: usize, value: u32) void {
    asm volatile (
        \\move.l %[dest], %%a0
        \\move.l %[value], (%%a0)
        :
...
// ---------------------------------------------------------------------------
// BIOS keyboard input (trap #13)
// ---------------------------------------------------------------------------

/// BIOS `Bconstat(2)` — true if a keyboard character is waiting.
pub fn keyPressed() bool {
    var r: i16 = 0;
    asm volatile (
        \\move.w #2, -(%%sp) // dev 2 = con:
...
/// BIOS `Bconin(2)` — read a keyboard character, normalized to the raw
/// scancode in the low byte (0x01 = Esc, 0x39 = Space, 0x4B/0x4D = arrows).
///
/// The ST TOS variants disagree on Bconin's return format:
/// - EmuTOS (this project's `tos.img`) returns just the scancode byte;
/// - TOS 1.x/2.x returns `(scancode << 8) | ascii`.
/// ASCII is always <= 0x7F, so a result <= 0xFF is the EmuTOS form; this
/// normalizes both to "scancode in the low byte".
pub fn readKey() i32 {
    var r: i32 = 0;
    asm volatile (
        \\move.w #2, -(%%sp) // dev 2 = con:
...
/// Hardware palette index (0-15), matching the EmuTOS/GEM desktop palette so
/// a fullscreen app needs NO palette install or restore (the desktop keeps its
/// palette and the game's colours adapt to it). The desktop palette is:
/// 0 white, 1 red, 2 green, 3 yellow, 4 blue, 5 magenta, 6 cyan, 7 grey,
/// 8-14 dark variants, 15 black. Bits 8-11 of a `te_color` word select one of
/// these for GEM text colours.
...
// TODO - is currently hardcoded, change this later so its user configurable ... probably create a .equalizer() helper that does all this
pub const eq_w: u16 = 232; // pixels
pub const eq_h: u16 = 12; // pixels
...
pub const GrafMouse = struct {
    ...
    pub const off: i16 = 256;
    pub const on: i16 = 257;
};
...
pub noinline fn windUpdate(mode: i16) void {
...
/// `appl_init` — register the application with the AES. Needed before
/// `graf_mouse` (mouse-cursor control) can be used.
pub fn applInit() i16 {
    const args = AesArgs.none();
    return aesCall(.appl_init, &args);
}

/// `appl_exit` — release the application from the AES. Fullscreen apps that
/// registered with `applInit()` MUST call this before GEMDOS exit (`pterm0`);
/// skipping it leaves the AES's application table inconsistent and the
/// desktop's resumption misbehaves (verified: a bus error in the desktop
/// redraw after the app exited).
pub fn applExit() void {
    const args = AesArgs.none();
    _ = aesCall(.appl_exit, &args);
}

pub noinline fn grafMouse(mode: i16) void {
    const int_in = [_]i16{mode};
...
pub fn App(comptime max_views: usize, comptime max_nodes: usize) type {
    return struct {
        ...
        pub fn eqRectangle(self: *Self, r: Rect) void {
            self.eq_rect = r;
        }
        ...
        fn eqTick(self: *Self) void {
            @memset(&eq_buf, 0xFF); // Fill buffer completely with solid bits
        ...
        pub fn run(self: *Self) void {
            ...
            while (true) {
                ...
                if (playing) {
                    ...
                    //         const v = &self.views[0];
                    //         windUpdate(WindUpdate.beg_update);
                    //         objcDraw(v.treeSlice(), @intCast(2), 0, self.eq_rect.x, self.eq_rect.y, self.eq_rect.w, self.eq_rect.h);
                    //         windUpdate(WindUpdate.end_update);
                    //     }
                    //     eq_draw += 1;
                    //     if (eq_draw > 20) eq_draw = 0;
                    // }
                }
            }
        }
...
/// LLVM lowers struct copies (e.g. returning a large struct by value) to a
/// `memcpy` call; provide our own 68000-safe one.
///
/// Writes go through `gemz.storeByte` (plain `(a0)` store) and the source is
/// walked with a many-pointer: the plain `dest[i] = src[i]` form is an indexed
/// byte store — the documented m68k miscompile (it can drop the index register
/// AND is pathologically slow in UAE; M68K_NOTES items 4/13). Verified: a
/// struct copy via the old form shifted the destination by 0x14 bytes and
/// corrupted the caller's code.
export fn memcpy(dest: [*]u8, src: [*]const u8, n: usize) [*]u8 {
    var i: usize = 0;
    var s = src;
    while (i < n) : (i += 1) {
        storeByte(@intFromPtr(dest) + i, s[0]);
        s += 1;
    }
    return dest;
}
...
/// to `memmove`; provide our own 68000-safe one. Same indexed-store avoidance
/// as `memcpy`, plus a backward pointer walk for the overlap case.
...
// ---------------------------------------------------------------------------
// VDI parameter block (trap #2, d0 = $73)
// ---------------------------------------------------------------------------

const VdiControl = extern struct {
    opcode: i16,
    n_pts_in: i16,
    n_pts_out: i16,
    n_int_in: i16,
...
const VdiPb = extern struct {
    control: *VdiControl,
    int_in: ?[*]const i16,
    pts_in: ?[*]const i16,
    int_out: [*]i16,
    pts_out: [*]i16,
    addr_in: ?[*]const ?[*]const u8 = null,
    addr_out: [*]?[*]u8 = undefined,
};

// ---------------------------------------------------------------------------
// Direct VDI Screen Blit for the Equalizer (`vro_cpyfm`, opcode 109)
// ---------------------------------------------------------------------------
const VdiMfdb = extern struct {
    addr: ?[*]const u8,
    w: i16,
    h: i16,
    wb: i16,
...
/// Blit the eq_buf directly to the window client area, bypassing the AES tree.
pub fn blitEqDirect(window: *const Window, r: Rect) void {
    const origin = window.workOrigin();
    const dest_x = origin.x + r.x;
    const dest_y = origin.y + r.y;

    const src_mfdb = VdiMfdb{
        .addr = &eq_buf,
        .w = r.w,
        .h = r.h,
        .wb = eq_row_bytes,
        .format = 1, // 1 = Standard 1-bit format (crucial for eq_buf)
        .col1 = 1, // Foreground color
        .col2 = 0, // Background color (transparent)
        .col3 = 0,
        .col4 = 0,
    };

    const dst_mfdb = VdiMfdb{
        .addr = null, // Target physical screen
        .w = 0,
        .h = 0,
        .wb = 0,
        .format = 0,
    ...
    const pts_in = [_]i16{
        ...
        r.w - 1,          r.h - 1,
        dest_x,           dest_y,
        dest_x + r.w - 1, dest_y + r.h - 1,
    };

    // vrt_cpyfm: int_in[0] = foreground color index, int_in[1] = background color index
    const int_in = [_]i16{ 1, 0 };

    var control = VdiControl{
        .opcode = 117, // vrt_cpyfm (Transparent monochrome blit)
        .n_pts_in = 4,
        .n_pts_out = 0,
        .n_int_in = 2,
        .n_int_out = 0,
        .subid = 0,
        .handle = 1, // Handle 1 is the default physical screen workstation handle
        .reserved = .{0} ** 4,
    };

    var int_out: [1]i16 = .{0};
    var pts_out: [4]i16 = .{0} ** 4;

    const addrin = [_]?[*]const u8{
        @ptrCast(&src_mfdb),
        @ptrCast(&dst_mfdb),
    };
    ...
    asm volatile (
        ...
        :
        : [pb] "r" (&pb),
        : .{ .memory = true, .ccr = true, .d0 = true, .d1 = true, .a0 = true, .a1 = true });
}
